...
class AssetLoader {
    ...
    async loadAssets() {
        ...
        try {
            ...
            // НЕМЕДЛЕННО создаем fallback
            this.createFallbackAssets();
        }
        
        // Гарантируем что кукла будет всегда
        this.ensureDollExists();
    ...
    async loadWardrobeConfig() {
        try {
            ...
            console.warn('⚠️ Не удалось загрузить конфиг, используем fallback');
    ...
    processConfig() {
        ...
        // Загружаем кукол (ОЧЕНЬ важно!)
        if (this.assets.wardrobeConfig.dolls) {
            this.assets.wardrobeConfig.dolls.forEach(doll => {
                if (doll.id && doll.image) {
                    this.assets.dolls[doll.id] = doll;
                    console.log(`✅ Кукла ${doll.id} добавлена`);
                }
            });
        }
    ...
    ensureDollExists() {
        // ГАРАНТИРУЕМ что кукла будет всегда!
        if (Object.keys(this.assets.dolls).length === 0) {
            console.log('🛠️ Создаем экстренную куклу');
            this.assets.dolls['barbie-base'] = this.createEmergencyDoll();
        }
    }

    createFallbackAssets() {
        console.log('🆘 Создаем экстренные ассеты');
        ...
        this.assets.wardrobeConfig = {
            ...
            dolls: [this.createEmergencyDoll()] // Важно!
        };
    ...
    // ... остальные методы createFallback... без изменений ...

    createDollSilhouette() {
        // Простая SVG кукла как fallback
        return `data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" width="300" height="500" viewBox="0 0 300 500">
            <rect width="300" height="500" fill="%23ffe6f2" rx="20"/>
            <circle cx="150" cy="100" r="40" fill="%23ffb6c1"/>
            <rect x="110" y="140" width="80" height="200" fill="%23ff69b4" rx="10"/>
            <path d="M110 340 L90 450 L120 450 L130 340" fill="%23ff69b4"/>
            <path d="M190 340 L210 450 L180 450 L170 340" fill="%23ff69b4"/>
            <circle cx="130" cy="90" r="8" fill="%2300aaff"/>
            <circle cx="170" cy="90" r="8" fill="%2300aaff"/>
            <path d="M130 120 Q150 140 170 120" stroke="%23ff4d8d" stroke-width="2" fill="none"/>
            <text x="150" y="480" text-anchor="middle" font-family="Arial" font-size="16" fill="%23ff4d8d">Барби</text>
        </svg>`;
    }

    getDoll(dollId) {
        const doll = this.assets.dolls[dollId] || this.assets.dolls['barbie-base'];
        if (!doll) {
            // Экстренный fallback
            return this.createEmergencyDoll();
        }
        return doll;
    }
